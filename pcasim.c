        ...
        *(loop++) = '\0';
        ...
        if (strcmp(key, "p") == 0) {
            ...
            for (double p = p_start_value; p < p_end_value; p += p_interval) {
                context->p = p;
                context->seed++;
                if (interpret(rest, context, table))
                    break;
            }
            ...
            for (char rule = rule_start_value; rule < rule_end_value;
                 rule += rule_interval) {
                context->rule = rule;
                context->seed++;
                if (interpret(rest, context, table))
            ...
            for (int width = width_start_value; width < width_end_value;
            ...
            for (int height = height_start_value; height < height_end_value;
            ...
            for (int seed = seed_start_value; seed < seed_end_value;
                 seed += seed_interval) {
                context->seed = seed;
                if (interpret(rest, context, table))
                    break;
...
int main(int argc, char **argv)
{
    ...
    while (1) {
        ...
        if (interpret(line, context, table) == -1)
            break;
