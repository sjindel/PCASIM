...
int sim_render(sim_table * table)
{
    for (int i = 0; i < table->h; i++) {
        ...
        while (c != NULL) {
            ...
            int *image =
                xcalloc(d.width * scale * d.height * scale, sizeof(int));
...
int sim_diff_hamm(sim_context * context, sim_table * table, char *args)
{
    ...
    sim_desc fix_desc =
        { fix_p, fix_rule, fix_width, fix_height, context->initial };
    ...
    if ((fix_desc.width != con_desc.width)
        || (fix_desc.height != con_desc.height)) {
...
{
    printf("rule: %u\np: %lf\nwidth: %d\nheight: %d\nseed: %d\ninitial:\n",
           context->rule, context->p, context->width, context->height,
           context->seed);
    for (int i = 0; i < context->width; i++)
